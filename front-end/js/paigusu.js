    ...
    g.prototype = {
        ...
        initDefaultColor: function() {
            ...
            if (1 == a.length) {
                ...
                e = this.rgbToHsb(b)
            } else 3 == a.length ? (b = a[0], d = a[1], e = a[2], a = 1) : 4 == a.length ? (b = a[0], d = a[1], e = a[2], a = a[3]) : (b = 42, d = 0, e = 255, a = 1),
            b = {
        ...
        getPosition: function() {
            ...
            d - e.bottom > a.height ? this.set({
                ...
                positionTop: (e.bottom - a.height) >= 0 ? (e.bottom - a.height) : 10
            })
        },
        doploy: function() {
            var a = this.options;
        ...
        on: function() {
            var a = this;
            c("#paigusu-screen").off().on("mousedown",
            function() {
                c(this).remove();
                return ! 1
            });
            c("#paigusu-color-picker").off().on("mousedown",
            function(a) {
                a.stopPropagation()
            });
            c("#paigusu-tab").off().on("click",
            function() {
                c("#paigusu-rgba").is(":hidden") ? (c("#paigusu-rgba").show(), c("#paigusu-rex").hide()) : (c("#paigusu-rgba").hide(), c("#paigusu-rex").show())
            });
            c("#paigusu-palette").off().on("mousedown",
            function(b) {
                b = b || f.event;
                f.paigusupalette = !0;
                a.set({
            ...
            c("#paigusu-slider").off().on("mousedown",
            function(b) {
                b = b || f.event;
                f.paigususlider = !0;
                a.set({
            ...
            c("#paigusu-alpha").off().on("mousedown",
            function(b) {
                b = b || f.event;
                f.paigusualpha = !0;
            ...
            c('input[name\x3d"paigusu-r"],input[name\x3d"paigusu-g"],input[name\x3d"paigusu-b"],input[name\x3d"paigusu-a"]').on("keyup",
            function() {
                var b = c(this).attr("name").split("-");
                if ("a" == b[1]) {
                    var d = parseFloat(c(this).val()) || 1,
                    d = parseFloat(d.toFixed(1));
                    if (1 < d || 0 > d) c(this).val(1),
                    d = 1
                } else d = parseInt(c(this).val()) || 0,
                0 > d ? (c(this).val(0), d = 0) : 255 < d && (c(this).val(255), d = 255);
            ...
            c('input[name\x3d"paigusu-hex"]').on("keyup",
            function() {
                var b = c(this).val().replace("#", "");
                if (3 == b.length || 6 == b.length) b = a.hexToRgb(b),
                b.a = 1,
                a.set({
                    color: b,
                    hex: a.rgbToHex(b),
